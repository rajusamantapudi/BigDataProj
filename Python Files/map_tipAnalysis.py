...
for line in sys.stdin:
    if len(line.split(',')) == 19:
        ...
        trip_distance = val[4]
        fare_amount = float(val[12])
        # print "fare: " + str(fare_amount)
        if fare_amount > 0:
            tip_amount = float(val[15])
            # print "tip: " + str(tip_amount)
            tip_percentage = int(round(tip_amount * 100 / fare_amount))
            print str(key) + ':' + str(tip_percentage)
    else:
        weather = {}
        YMD = line[13:21]
        if YMD == 'YR--MODA':
